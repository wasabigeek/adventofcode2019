...
def xvaporisation_order(chart):
    start = get_best_pos(chart)
    asteroids = set(get_asteroids(chart))
    # filter out start
    ...
    while len(asteroids) > 0:
        # print(len(asteroids))
        visibles = _find_visibles(asteroids, start)
    
        right_half = []
        left_half = []

        for position in visibles:
            normalised = normalise(position)
            # right half
            if (normalised[0] >= 0 and normalised[1] >= 0) or (normalised[0] > 0 and normalised[1] < 0):
                right_half.append(position)
            else:
                left_half.append(position)
    
        sorted_right = sorted(
            right_half,
            key=lambda p: LineEquation((0, 0), normalise(p)).gradient,
            reverse=True
        )
        sorted_left = sorted(
            left_half,
        ...
        asteroids = asteroids - set(visibles)
        order += (sorted_right + sorted_left)
        
    # [print(LineEquation((0,0), normalise(p)).gradient, p) for p in order]
