...
def execute(_intcode, inputs):
    ...
    is_paused = False
    while index < len(intcode) and not is_paused:
        op = _get_op(intcode[index])
        # print("current code:", intcode[index], ", op:", op, ", inputs:", inputs)
        if op:
    ...
    return intcode, diagnostic, index, is_paused


class Amp(object):
    def __init__(self, initial_intcode=(), phase_setting=None):
        self.intcode = list(initial_intcode)
        self.index = 0
    ...
    def execute(self, inputs=[]):
        ...
        if self.phase_setting:
            inputs = [self.phase_setting] + inputs
            self.phase_setting = None
    
        self.is_paused = False
        while not self.is_paused and self.index < len(self.intcode):
            op = _get_op(self.intcode[self.index])
            # print("current code:", intcode[index], ", op:", op, ", inputs:", inputs)
            if op:
                try:
                    self.index, output = op(self.index, self.intcode, inputs=inputs)
...
def calc_thrust(_intcode, settings):
    output = 0
    amps = deque([Amp(initial_intcode=_intcode, phase_setting=a) for a in settings])
    
    while len(amps) > 0:
        amp = amps.popleft()
        output = amp.execute(inputs=[output])
        print(output)
